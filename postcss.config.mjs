...
function Emblem({ category, rng, glow, ink, accent }: { category: Category; rng: () => number; glow: string; ink: string; accent: string }) {
    ...
    <g stroke={ink} strokeWidth={3} fill="none">
      ...
      {Array.from({ length: 12 }).map((_, i) => {
        ...
        return (
          <line
            key={i}
            x1={200 + Math.cos(a) * 56}
            y1={250 + Math.sin(a) * 56}
            x2={200 + Math.cos(a) * 72}
            y2={250 + Math.sin(a) * 72}
            stroke={glow}
            strokeWidth={2}
          />
        );
    ...
    <g>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        return (
          <path
            key={i}
            d={`M200 250 L${200 + Math.cos(a) * 100} ${250 + Math.sin(a) * 100} L${200 + Math.cos(a2) * 42} ${250 + Math.sin(a2) * 42} Z`}
            fill="none"
            stroke={glow}
            strokeWidth={3}
          />
    ...
    <g fill="none" stroke={glow} strokeWidth={2.5}>
      ...
      {Array.from({ length: 7 }).map((_, i) => {
        ...
        return (
          <line
            key={i}
            x1={200 + Math.cos(a) * 84}
            y1={250 + Math.sin(a) * 84}
            x2={200 + Math.cos(b) * 84}
            y2={250 + Math.sin(b) * 84}
          />
        );
      })}
  ...
  const tee = (
    <g fill={ink} fillOpacity={0.92}>
      ...
      <g transform="translate(200 250) scale(0.55)">
        {(() => {
          const inner = rng();
          if (inner < 0.34) return eye;
          if (inner < 0.67) return star;
          return moon;
        })()}
      </g>
    </g>
  ...
  switch (category) {
    case "Art Prints": {
      const pick = rng();
      return pick < 0.2 ? eye : pick < 0.4 ? moon : pick < 0.6 ? star : pick < 0.8 ? sun : hand;
    }
    case "Resin & Epoxy":
      return resin;
    case "Custom Tees":
    ...
    case "Ouija Boards": {
      const pick = rng();
      return pick < 0.5 ? planchette : sigil;
    }
    case "Jewelry": {
      const pick = rng();
      return pick < 0.5 ? ring : star;
    }
    case "Novelty": {
      const pick = rng();
      return pick < 0.4 ? candle : pick < 0.7 ? sigil : moon;
    }
    default:
      return eye;
  }
